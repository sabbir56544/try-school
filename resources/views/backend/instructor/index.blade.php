            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                        <h2><strong>Instructor</strong> List </h2>
                    </div>
                    <div class="card-body">
                        <div class="body">
                            <div class="table-responsive">
                                <table id="exhibition_table" class="table table-bordered table-hover">
                                    ...
                                    <tbody>
										...
										<tr>
											<td>{{ $s->id }}</td>
											<td><a href="{{ route('instructor.show', $s->id) }}">{{ $s->first_name }} {{ $s->last_name }}</a></td>
											<td>{{ $s->email }}</td>
                                            <td>{{ $s->phone }}</td>
											@if ($s->image)
											<td>
												<img src="{{ url('uploads/images/instructor', $s->image)}}" style="width: 60px" height="50px">
											</td>
											@else
											<td>
											...
											@endif
											<td>
												<a href="{{ route('instructor.edit', $s->id) }}" class="btn btn-primary btn-sm">Edit</a>
												<form action="{{ route('instructor.destroy', $s->id) }}" method="POST" style="display: inline-block;">
													@csrf
													@method('DELETE')
